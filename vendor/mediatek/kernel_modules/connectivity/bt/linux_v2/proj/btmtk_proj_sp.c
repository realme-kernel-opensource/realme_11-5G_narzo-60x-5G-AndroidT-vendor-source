...
static int btmtk_read_register(struct btmtk_dev *bdev, u32 reg, u32 *val)
{
	...
	if (ret < 0) {
		BTMTK_ERR("%s: btmtk_main_send_cmd failed, ret=%d", __func__, ret);
		return ret;
	}
	...
	if (bdev->io_buf) {
		...
		*val = le32_to_cpu(*val);
		...
		ret = -1;
		BTMTK_ERR("%s: io_buf already freed", __func__, ret);
		return ret;
	}
...
void bthost_read_tx_power(void)
{
	...
	if (cif_dev->own_state == BTMTK_DRV_OWN) {
		err = btmtk_read_register(g_sbdev, BT_TX_POWER_TSSI, &val);
		if (err < 0) {
			BTMTK_ERR("btmtk_read_register failed!");
			return;
		}
