...
EXPORT_SYMBOL(oplus_chg_get_otg_online);
...
int oplus_set_chr_enable_otg(bool enable)
{
	if (!g_charger_chip) {
	...
	if (!g_charger_chip->chg_ops || !g_charger_chip->chg_ops->otg_enable || !g_charger_chip->chg_ops->otg_disable) {
		printk(KERN_ERR "[oplus_CHG][%s]: g_oplus_chip->chg_ops otg failed.\n", __func__);
		return -1;
	}

	if (enable) {
		g_charger_chip->chg_ops->otg_enable();
	} else {
		g_charger_chip->chg_ops->otg_disable();
	}
	return 0;
}
EXPORT_SYMBOL(oplus_set_chr_enable_otg);
